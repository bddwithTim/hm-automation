...
class TestDict(object):
    ...
    def create_xml_report(tc_dict):
        ...
        envnode.appendChild(doc.createElement("timestopped")).appendChild(
            doc.createTextNode("tbd: yyyy-mm-dd_hh.mm.ss"))
        ...
        for tc_key in tc_keys:
            ...
            tsNode.setAttribute("title", tc_subdict.get("title"))
            tsNode.setAttribute("txt_color", tc_subdict.get("txt_color"))
            tsNode.setAttribute("suite", tc_subdict.get("suite"))
            total_pass += int(tc_subdict.get("passed"))
            total_fail += int(tc_subdict.get("failed"))
            total_time += float(tc_subdict.get("duration"))
        # endfor

        total_time = format(total_time, ".3f")
        summaryNode = summary.appendChild(doc.createElement("test_summary"))
        summaryNode.appendChild(doc.createElement("totaltest")).appendChild(
            doc.createTextNode(str(total_pass + total_fail)))
    ...
    def create_xml(self, report_dict):
        ...
        """
        ts_keys = list(report_dict.keys())
        ts_keys.sort()

        tr_dir = os.path.join(fm.workingdir, fm.reportfolder)
...
# if __name__ == '__main__': test = TestDict() print test.testDict test.testDict = {"testsuite":"regression",
# "testcase":"testcase7", "passed":"1", "failed":"0", "duration":"77.894999980926514", "browser":"ie"} print
# test.testDict test.testDict = {"testsuite":"regression", "testcase":"testcase8", "passed":"0", "failed":"1",
# "duration":"88.743000030517578", "browser":"chrome"} print test.testDict test.testDict = {"testsuite":"regression",
#  "testcase":"testcase9", "passed":"1", "failed":"0", "duration":"99.743000030517578", "browser":"safari"} print
# test.testDict test.testDict = {"testsuite":"endtoend", "testcase":"testcase11", "passed":"0", "failed":"1",
# "duration":"111.894999980926514", "browser":"firefox"} print test.testDict test.create_xml(test.testDict)
