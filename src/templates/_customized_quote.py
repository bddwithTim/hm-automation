...
class _CustomizedQuote(prep):
    ...
    def broker_census_section(self):
        ...
        self.p.act(self.file_, self.click, 'View Plans', '<view_plans_bttn>')

    def plan_compare_section(self):
        self.p.act(self.file_, self.verify, 'Displayed:Plan Compare page', 'form#frmPlanCompare')
        self.p.act(self.file_, self.click, 'Send All Quoted to Prospect', '<uho_send_all_quoted_prospect_bttn>')
    ...
    def prospect_information_section(self):
        ...
        self.p.act(self.file_, self.click, 'Send to Prospect', '<uho_send_to_prospect_bttn>')
        self.p.act(self.file_, self.verify, 'Expected:Send to Prospect', '<uho_prospect_information_lbl>')
        # self.p.act(self.file_, self.capture, 'unique_id', '[for=lblQuoteUniqueIdDisplay]')
        self.p.act(self.file_, self.click, 'Return to Quoting', '<uho_return_to_quoting_bttn>')

    def home_section(self):
        self.p.act(self.file_, self.verify, 'Expected:Quoting & Application', '<uho_quoting_application_page>')
        self.p.act(self.file_, self.click, 'Sign Off', '<uho_sign_off_lnk>')
        self.p.act(self.file_, self.verify, 'Expected:Welcome Brokers!', '<uho_brokers_login_page>')

    def email_section(self):
        self.p.act(self.file_, self.check, 'Email:' + base.ts_data['primaryfname'] + ' '
                   + base.ts_data['primarylname'], 'UHOne')

    def plan_compare_2_section(self):
        # self.p.act(self.file_, self.verify, 'Displayed: Discount Card tab', '<uho_discount_card_tab>')
        tab = ' tab'
        ...
        for i, plan in enumerate(base.ts_data['plans']):
            if self.plan_stm in plan:
                ...
                if self.plan_vision in plan:
                    ...
                    # self.p.act(self.file_, self.click, plan + ' Add to Cart', self.elmnt_uho_add_to_cart)
            if base.ts_data['zipcode'] == '44110' and self.plan_discount in plan:
    ...
    def shopping_cart_section(self):
        ...
        if page.has_consecutive_stm_pop_up(base.ts_data['state'], base.ts_data['plans']):
            for i, plan in enumerate(base.ts_data['plans']):
                if self.plan_stm in plan:
                    other_options = base.ts_data['plandict'][plan][
                        self.plan + str(i + 1) + ' ' + str(self.planotheroptions).lower().strip()]
    ...
    def applicant_info_section(self):
        ...
        for i, applicant in enumerate(base.ts_data['applicants']):
            ...
            if i in ssn or (base.ts_data['zipcode'] == '55473' and self.plan_hsg in plan):
    ...
    def review_submit_section(self):
        self.p.act(self.file_, self.verify, 'Displayed: Review and Submit page',
                   self.elmnt_uho_review_submit_header_page, '', 0)
        ...
        #         fact = self.p.get_attrib_from_notes(self.plans, self.plandict, 'fact')
        #         if fact:
        ...
        for plan in base.ts_data['plans']:
            if self.plan_hpg in plan:
                self.p.act(self.file_, self.click, self.plan_hpg + ' checkbox',
                           self.elmnt_uho_e_signature + str(i) + self.gt, '', 0)
                i += 1
            elif self.plan_stm in plan:
                ...
                if fact:
                    self.p.act(self.file_, self.click, 'FACT checkbox', self.elmnt_uho_e_signature_fact, '', 0)
                i += 1
            elif self.plan_dental in plan:
                ...
                for plan in base.ts_data['plans']:
                    if page.has_signature_checkbox(base.ts_data['zipcode'], plan, i):
                        self.p.act(self.file_, self.click, 'Applicant Signature ' + str(i + 1),
                                   self.elmnt_uho_app_e_signature_bttn + str(i + 1) + self.gt, '', 0)
                        break
        # self.p.act(self.file_, self.click, 'submit your application', '<uho_submit_your_application_bttn>')
        self.p.act(self.file_, self.click, 'submit your application', self.elmnt_uho_continue_bttn)

    def thank_you_section(self):
        self.p.act(self.file_, self.wait, 'Uho Loading...', self.elmnt_uho_loading_img, 'm')
