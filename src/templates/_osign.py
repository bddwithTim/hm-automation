...
class _OSign(prep):
    ...
    def intro_section(self):
        ...
        self.p.act(self.file_, self.url, base.ts_data['uri'], 'UnitedHealthOne Broker Portal')
        # self.p.act(self.file_, self.url, base.ts_data['uri'], '<uho_title_baa>')

    def welcome_brokers_section(self):
        self.p.act(self.file_, self.enter, base.ts_data['brokerid'], '<uho_osign_broker_id_fld>')
        self.p.act(self.file_, self.click, 'Submit', '<uho_osign_submit_login_btn>')
    ...
    def applicant_info_section(self):
        ...
        for plan in base.ts_data['plans']:
            if self.plan_critical in plan or self.plan_asg in plan or self.plan_hsg in plan or self.plan_tl in plan or self.plan_hpg in plan:
        ...
        self.p.act(self.file_, self.click, 'Complete App as Osign', 'button[type=button][data-ng-click*=OralSignature]')
        # if page.has_registration_pop_up(base.ts_data['zipcode']):
        #     self.p.uho_registration_dialog(self.file_)
        self.p.act(self.file_, self.verify, 'Expected: oSign Confirmation', '.modal-header .modal-title')
        self.p.act(self.file_, self.select, 'osign: all yes', 'a[role=radio][data-uib-btn-radio=true]')
        # self.p.act(self.file_, self.select, 'Questions: all no', 'a[role=radio][id*=\'_No\']')
        self.p.act(self.file_, self.enter, self.applicantphonenum, 'input#oSignPhone')
        self.p.act(self.file_, self.enter, 'Verifier', 'input#oSignVerifierame')
        self.p.act(self.file_, self.click, 'Save and Close', 'input[type=submit]')
        self.p.act(self.file_, self.verify, 'Displayed: Applicant information page',
                   self.elmnt_uho_app_info_header_page,
                   '', 0)
        self.p.act(self.file_, self.click, 'Continue', self.elmnt_uho_continue_bttn)
    ...
    def review_submit_section(self):
        ...
        for plan in base.ts_data['plans']:
            if self.plan_hpg in plan:
                ...
                i += 1
        # checkbox for O-Sign
        self.p.act(self.file_, self.click, 'O-Sign checkbox', self.elmnt_uho_o_signature, '', 0)
        for i, applicant in enumerate(base.ts_data['applicants']):
            if i > 1:
                break
