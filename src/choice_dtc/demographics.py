...
class Demographics:
    ...
    def fill_out_details(self, insurance_type: str, locator_type: str, phone: str = None,
                         email: str = None,
                         ...
                         medicare_coverage_year: str = None) -> None:
    ...
    def _fill_out_stm(self, data: dict, locator_type: str) -> None:
        ...
        dob = self.browser.get_web_element(data['date_of_birth'][parse_str(locator_type)],
                                           locator_type)
        _clean_textbox_data(dob)
        dob.send_keys(self.date_of_birth)
        # click the dropdown listbox
        self.browser.get_web_element(data['gender'][parse_str(locator_type)],
                                     locator_type).click()
        # click the option from the dropdown listbox
        self.browser.get_web_element(data['gender'][self.gender.lower()],
                                     locator_type='xpath').click()
        email = self.browser.get_web_element(data['email'][parse_str(locator_type)],
                                             locator_type)
        _clean_textbox_data(email)
        email.send_keys(self.email)
        self.browser.get_web_element(data['parent_radio_button'][f'{self.parent.lower()}_option'],
                                     locator_type='xpath').click()
        if not is_legal_age(self.date_of_birth):
            self.browser.get_web_element(data['tobacco_radio_button'][f'{self.parent.lower()}_option'],
                                         locator_type='xpath').click()
    ...
    def _fill_out_medicare(self, data: dict, locator_type: str) -> None:
        ...
        phone = self.browser.get_web_element(data['phone'][parse_str(locator_type)],
                                             locator_type)
        ...
        _clean_textbox_data(email)
        email.send_keys(self.email)
        first_name = self.browser.get_web_element(data['first_name'][parse_str(locator_type)],
                                                  locator_type)
        _clean_textbox_data(first_name)
        first_name.send_keys(self.first_name)
        last_name = self.browser.get_web_element(data['last_name'][parse_str(locator_type)],
                                                 locator_type)
    ...
    def _fill_out_aca(self, data: dict, locator_type: str) -> None:
        ...
        if self.annual_income is not None:
            annual_income = self.browser.get_web_element(data['annual_income'][parse_str(locator_type)],
                                                         locator_type)
            _clean_textbox_data(annual_income)
            annual_income.send_keys(self.annual_income)

            household_members = self.browser.get_web_element(data['household_members'][parse_str(locator_type)],
                                                             locator_type)
